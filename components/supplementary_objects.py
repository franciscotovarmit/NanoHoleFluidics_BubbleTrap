...
class Triangle(i3.PCell):
    _name_prefix = "TRIANGLE"  # a prefix added to the unique identifier

    class Layout(i3.LayoutView):
        channel_template = microfluidics.ShortChannelTemplate()
        side = i3.NumberProperty(default=1000)
        radius  =  i3.NumberProperty(default=100)
        ...
        def _generate_elements(self, insts):
            ...
            point_list.insert(0,  (0., self.side))
            point_list.append( (self.side, 0.))

            t = i3.Shape(point_list, closed=True)
            rounded_shape = i3.ShapeRoundAdiabaticSpline(original_shape= t,
                                                         radius= self.radius,
                                                         adiabatic_angles=(10.0, 10.0))
            bo = i3.Boundary(layer = self.layer, shape = rounded_shape)

            s= i3.Structure(elements = bo)
            insts += i3.SRef(s)
            return insts
    ...
    class Layout(i3.LayoutView):
        ...
        def _generate_elements(self, elems):
            elems += i3.CirclePath(layer=i3.TECH.PPLAYER.CH2.TRENCH,
                                        center=(0.0,0.0),
                                        radius=self.size*0.5, line_width = 100)
            return elems
